...
const SubCategory = () => {
  ...
  useEffect(() => {
    ...
    if (selectedSubCategory && selectedSubCategory.categoryId) {
      ...
      const category = categories.find((cat) => cat._id === selectedSubCategory.categoryId);
  ...
  const handleEditClick = (subCategory) => {
    ...
    const category = categories.find((category) => category._id === subCategory.categoryId);
  ...
  const handleSubmit = async (e) => {
    ...
    const subCategoryData = { subCategoryName, status, categoryId };
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to process sub category");
    }
  };
  ...
  const handleUpdateSubCategory = async (e) => {
    ...
    if (!selectedSubCategory.subCategoryName || !selectedSubCategory.status || !selectedCategory) {
      toast.error("Sub Category Name, Status, and Category are required.");
      return;
    }
    ...
    try {
      ...
      toast.error("An error occurred while updating the sub category. Please try again.");
    }
  };
  ...
  const handleCategoryChange = (selectedOption) => {
    if (selectedOption) {
      ...
      const selectedCat = categories.find((category) => category._id === selectedOption.value);
                  ...
                  <div className="col-lg-12">
                    <div className="form-row">
                      ...
                      <Select
                          ...
                          placeholder="Select Categories"
                          onChange={(selectedOption) => handleCategoryChange(selectedOption)} // Correctly handle the change event
                          value={selectedSubCategory?.category
                              ? {
                                label: selectedSubCategory.category.categoryName, // Display the selected category name
                                value: selectedSubCategory.category._id, // The category ID is set as the value
                              }
                              : null} // Correctly set selected category based on `selectedSubCategory.category`
