  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing && selectedSubCategory) {
        ...
        // Add new subcategory
        response = await axios.post("http://localhost:5070/api/v1/sub-category", subCategoryData);
      ...
      const updatedSubCategories = isEditing
          ? subCategories.map(subCategory =>
              subCategory._id === selectedSubCategory._id ? response.data.data : subCategory
          )
      ...
      if (data && data._id) {
        ...
        setSubCategories((prevSubCategories) =>
            prevSubCategories.map((subCategory) =>
                subCategory._id === data._id ? { ...subCategory, ...data } : subCategory
            )
        );
                  ...
                  {filteredSubCategories.length > 0 ? (
                      filteredSubCategories.map((subCategory, index) => (
                          <tr key={subCategory._id || index}>
                            <td>{index + 1}</td>
                            <td>{subCategory?.subCategoryName || "Sub Category Name Not Found!"}</td>
                            <td>
                              <span className={subCategory?.status === "active" ? "active" : "inactive"}>
                  ...
                  <div className="col-lg-12">
                    ...
                    <div className="form-row select-input-box">
                      ...
                      <select
                          ...
                          onChange={(e) => setSelectedSubCategory({...selectedSubCategory, status: e.target.value})}
                      >
                        <option value="">Select Status</option>
                        <option value="active">Active</option>
                        <option value="inactive">Inactive</option>
